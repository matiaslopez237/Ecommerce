...
router.get("/", async (req, res) => {
    ...
    if (req.query.providerPaymentId)
        where.providerPaymentId = String(req.query.providerPaymentId);
    if (req.query.preferenceId)
        where.preferenceId = String(req.query.preferenceId);
    if (req.query.merchantOrderId)
        where.merchantOrderId = String(req.query.merchantOrderId);
    if (req.query.orderId)
        where.orderId = Number(req.query.orderId);
    if (req.query.userId)
        where.order = { userId: Number(req.query.userId) };
